...
def get_arguments() -> tuple[str, str]:
    ...
    if "-d" in args:
        d_idx = args.index("-d")
        # Encontra o fim dos argumentos de diretório
    ...
    if "-f" in args:
        f_idx = args.index("-f")
        # Verifica se existe um nome de arquivo após a flag -f
        if f_idx + 1 < len(args):
...
def get_content_from_user() -> str:
    ...
    # Formatação: timestamp + conteúdo numerado
    return timestamp + "\n" + "\n".join(content_lines) + "\n"
...
def write_to_file(dir_path: str, file_name: str, content: str) -> None:
    ...
    # Adiciona linha em branco apenas se o arquivo já existir
...
def main() -> None:
    ...
    # A lógica de conteúdo só roda se um NOME de arquivo foi passado
    if file_name:
