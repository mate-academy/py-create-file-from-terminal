...
def main() -> None:
    if len(sys.argv) < 3:
        raise ValueError("Usage: python create_file.py -d <directory_path_parts> -f <file_name>")

    if "-d" in sys.argv:
        dir_index = sys.argv.index("-d") + 1
        create_dir(sys.argv[dir_index:sys.argv.index('-f')] if '-f' in sys.argv else sys.argv[dir_index:])
