...
def create_path_to_folders(path_data: list) -> str:
    input_list = []
...
def create_path_to_file(path_data: list) -> str:
    return os.path.join(path_data[path_data.index("-f") + 1])


def create_only_file(input_data: list) -> None:
    add_new_content_line(create_path_to_file(input_data))


def create_only_folders(input_data: list) -> None:
    os.makedirs(create_path_to_folders(input_data), exist_ok=True)


def create_folders_and_file(input_data: list) -> None:
