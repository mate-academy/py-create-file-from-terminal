...
def create_file_command(args: list) -> None:
    dirs = ""
    command_d = args.index("-d")
    command_f = args.index("-f")

    if "-d" in args:
        if "-f" not in args or (
                "-f" in args and command_d > command_f):
            dirs = os.path.join(*args[command_d + 1:]) + "/"
        else:
            dirs = os.path.join(
                *args[command_d + 1:command_f]
            ) + "/"
    ...
    if "-f" in args:
        ...
        with open(dirs + args[command_f + 1], "a") as file:
